...
function callableModel(defaultExport: boolean): ProviderDeclarationModel {
  return {
    moduleSpecifier,
    ...
    exports: [{
      ...
      name: "select",
      kind: "function",
      ...(defaultExport ? { exportKind: "default" as const } : {}),
      signatures: [
      ...
      members: [{
        ...
        id: "select::version", name: "version", kind: "property", readonly: true, type: { kind: "string" },
      }],
    }],
  };
}
...
for (const defaultExport of [false, true]) {
  test(`provider callable namespace members retain one export and exact overload identities, default=${defaultExport}`, () => {
    const model = callableModel(defaultExport);
    const session = createCompilerSessionFromFiles({
      ...
      files: {
        ...
        "/src/index.ts": `
          import ${defaultExport ? "choose" : "{ select as choose }"} from "${moduleSpecifier}";
